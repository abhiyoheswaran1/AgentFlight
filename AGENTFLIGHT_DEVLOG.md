...
- After ready artifact discovery changed to prefer the handoff packet,
  clean-worktree status still said tucked verification details could be found by
  opening report/replay or JSON output. That was accurate, but it omitted the
  new first artifact and made the status copy less consistent with the local
  handoff path.

Implemented locally:

- Clean-worktree status tucked-verification copy now says to open
  handoff/report/replay or JSON output for the full ledger.
- The condition for tucking details is unchanged; unresolved failed
  verification still keeps details visible.

Verification so far:

- Red AgentFlight-captured
  `npm test -- tests/commands/evidence-output.test.ts` failed on the old
  `open report/replay or JSON` wording.
- Green AgentFlight-captured
  `npm test -- tests/commands/evidence-output.test.ts` passed with 1 file / 35
  tests.
...
- In a clean temporary first-run app, `.projscan-memory/memory.json` no longer
  dominated Review Focus. The remaining friction was generated
  `.agentflight/.gitignore`, which ranked ahead of a real README change because
  it shared the same AgentFlight config score as `.agentflight/config.json`.

Team persona notes:

- Product Maintainer: first-run review focus should lead with the project
  config and real app change, not helper files generated by the tool.
- CLI Engineer: keep this in shared Review Intelligence so status, handoff,
  report, replay, and resume stay aligned.
- Docs and DX Writer: keep `.agentflight/.gitignore` visible and explain its
  purpose without making it look like the main review target.
- Verification Engineer: add both core ranking and command-output regression
  coverage.
- Security Reviewer: do not hide generated local files or change upload,
  telemetry, or artifact behavior.

Implemented locally:

- `.agentflight/.gitignore` now uses generated-helper Review Intelligence
  scoring, so `.agentflight/config.json` and real app changes rank above it.
- `.projscan-memory/memory.json` remains suggestion-only guidance and is not a
  built-in ignored path.
- The generated helper keeps AgentFlight-specific reviewer focus copy instead
  of reusing ProjScan memory guidance.

Verification so far:

- Red AgentFlight-captured
  `npm test -- tests/core/review-intelligence.test.ts tests/commands/evidence-output.test.ts`
  failed because `.agentflight/.gitignore` ranked above the real app change.
- Green AgentFlight-captured
  `npm test -- tests/core/review-intelligence.test.ts tests/commands/evidence-output.test.ts`
  passed with 2 files / 59 tests.
- AgentFlight-captured `npm run verify` passed with 22 files / 209 tests plus
  build.
- AgentFlight-captured `npm run format:check` initially failed on the touched
  TypeScript files; Prettier fixed those files and the rerun passed.
- AgentFlight-captured `npm pack --dry-run` passed for `agentflight@0.6.0`.
- ProjScan doctor passed with health `100/A`.
- ProjScan preflight returned the known accumulated branch scale caution and
  asked for full review.
- ProjScan review returned the known scale-only `block` verdict with 49 current
  ...
  flows, or dataflow risks.
- AgentFlight-captured `npx agentloopkit@latest verify` passed and wrote
  `.agentloop/reports/2026-06-21-14-12-verification-report.md`.
- Rebuilt-CLI temp-app dogfood showed Review Focus in the intended order:
  `.agentflight/config.json`, `README.md`, `.agentflight/.gitignore`, then
